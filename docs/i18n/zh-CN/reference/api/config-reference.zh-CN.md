...
| 键 | 默认值 | 用途 |
|---|---|---|
| `max_images` | `4` | 待处理用户轮次中 `[IMAGE:]` 标记数量上限（见注意事项） |
| `max_image_size_mb` | `5` | base64 编码前的单图像大小限制 |
| `allow_remote_fetch` | `false` | 允许从标记中获取 `http(s)` 图像 URL |

注意事项：

- `max_images` 适用于末尾连续用户消息块（位于最后一条非用户消息（如助手或工具）之后）。当该块包含新的图像标记时，更早的用户消息会移除 `[IMAGE:]`（保留文字说明；仅含图像的旧轮次会变为 `[earlier image omitted]`）。纯文本跟进会保留历史中的先前图像，且不会针对整条历史再次套用上限。
